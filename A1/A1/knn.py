...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.reshape(num_train, -1)
    x_test_flat = x_test.reshape(num_test, -1)
    train_sq = x_train_flat.pow(2).sum(dim=1, keepdim=True)  # (num_train, 1)
    test_sq = x_test_flat.pow(2).sum(dim=1, keepdim=True)    # (num_test, 1)
    cross = x_train_flat.mm(x_test_flat.t())                 # (num_train, num_test)
    dists = train_sq - 2 * cross + test_sq.t()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        _, nearest_idx = dists[:, j].topk(k, largest=False)
        closest_y = y_train[nearest_idx]
        y_pred[j] = torch.bincount(closest_y).argmax()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            acc = classifier.check_accuracy(x_val_cv, y_val_cv, k=k, quiet=True)
