...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    N = x_tensor.shape[0]
    C = 1 + x_tensor.max().item()
    y = torch.zeros(N, C, dtype=torch.float32)
    y[torch.arange(N), x_tensor] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    cat_x = torch.cat(xs)
    cumsum = torch.cumsum(cat_x, dim=0)
    end_idx = torch.cumsum(ls, dim=0) - 1
    end_sums = cumsum[end_idx]
    shifted = torch.cat(
        [torch.zeros(1, dtype=end_sums.dtype, device=end_sums.device), end_sums[:-1]]
    )
    segment_sums = end_sums - shifted
    y = segment_sums / ls.to(segment_sums.dtype)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
